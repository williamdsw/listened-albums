...
function listAll(item, toShow) {
  ...
  // New year container
  yearContainer.innerHTML += `<div id="year_accordion_${
    item.year
  }" class="accordion">
        <div class="card">
            <div id="accordion_header_${item.year}" class="card-header">
                <h1 class="mb-0"> ${item.year} - ${
    contentLength >= 1 ? contentLength + " album(s) " : "Not found"
  } </h1>
            </div>

            <div id="accordion_content_${
              item.year
            }" class="card-content collapse ${toShow ? "show" : ""}">
                <div class="card-body">
  ...
  item.albums.forEach((album) => {
    ...
    const linkTag = album.streamLink
      ? `<a class="listen" href="${album.streamLink}" target="_blank"> Listen </a>`
      : "";
    albumContainer.innerHTML += `<div class="album col-lg-3 col-md-3 col-xs-6 col-sm-6 p-2">
...
function filterList(property, searchString) {
  ...
  albumHeader.innerHTML =
    total >= 1
      ? `${total} occurrence(s) found including '${searchString}'`
      : "Nothing was found";
